...
_KEY_MAP = {
    "alt_r": keyboard.Key.alt_r,        # Right Option on macOS
    "alt_l": keyboard.Key.alt_l,        # Left Option
    "ctrl_r": keyboard.Key.ctrl_r,
    "ctrl_l": keyboard.Key.ctrl_l,
    "cmd_l": keyboard.Key.cmd_l,        # Left Command (⌘)
...
# pynput's Mac backend may not expose `fn` on all versions — drop if None.
_KEY_MAP = {k: v for k, v in _KEY_MAP.items() if v is not None}


class HotkeyListener:
    """Global hotkey listener. Emits PRESS/RELEASE for a single key or combo.

    Pass a string for a single key (e.g. ``"alt_r"``) or a list for a combo
    (e.g. ``["cmd_l", "space"]``). For combos, PRESS fires when ALL keys are
    held; RELEASE fires when ANY key in the combo is released afterward.

    Note (macOS): pynput requires Accessibility permission. Grant via
    System Settings → Privacy & Security → Accessibility, add the Python
    binary (or the Terminal you're running from).
    """
