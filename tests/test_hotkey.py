...
def test_combo_fires_only_when_all_keys_pressed():
    """Ctrl+Space combo: PRESS fires only when BOTH keys are held."""
    events = []
    listener = HotkeyListener(
        keys=["ctrl_l", "space"], mode="hold", on_event=events.append
    )
...
def test_combo_key_repeat_ignored():
    """Holding a combo key down (key repeat) shouldn't re-fire PRESS."""
    events = []
    listener = HotkeyListener(
        keys=["ctrl_l", "space"], mode="hold", on_event=events.append
    )
